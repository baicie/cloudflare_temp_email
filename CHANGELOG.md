...
- feat: |Admin| 创建邮箱支持单个/批量模式切换；批量模式可配置名称前缀、起始序号和最多 100 个地址，显示地址范围与创建进度，单项失败不会中断后续任务，并可复制或下载包含 Address JWT 的 CSV 结果
...
- fix: |Worker| 启用 Worker 静态资源部署时保留 `/health_check` 后端路由，避免健康检查被 SPA fallback 接管并返回前端 HTML
