...
- feat: |Admin| Add single/batch modes to address creation. Batch mode accepts a name prefix, starting number, and up to 100 addresses; shows the generated range and progress; continues after individual failures; and supports copying or downloading CSV results containing Address JWTs
...
- fix: |Worker| Preserve the `/health_check` backend route when Worker static assets are enabled, preventing the SPA fallback from returning frontend HTML for health checks
